...
export default function ConstructionSubcategoryPage() {
    const { subcategory } = useParams<{ subcategory: string }>()
    const [params, setParams] = useState(
        new URLSearchParams({ page: "1", limit: "20", sort: "newest", subcategory })
    )
    const query = useMemo(() => params.toString(), [params])
    const { services, pagination, loading, error } = useConstructionServices(query)
    ...
    return (
        <main className="mx-auto max-w-7xl px-4 py-8">
            <Link href="/construction-freelancers" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-primary transition-colors mb-4">
                <ChevronLeft className="h-4 w-4" />All construction freelancers
            </Link>

            <div className="mb-6">
                <h1 className="text-3xl font-bold text-foreground">{displayName}</h1>
                <p className="text-muted-foreground mt-1">Find {displayName.toLowerCase()} professionals in Kenya</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="py-16 text-center text-muted-foreground">Failed to load freelancers.</div>
            ) : services.length === 0 ? (
                <div className="py-16 text-center">
                    <p className="text-muted-foreground mb-4">No {displayName.toLowerCase()} found.</p>
                    <Button variant="outline" asChild><Link href="/construction-freelancers">Browse all freelancers</Link></Button>
                </div>
            ) : (
